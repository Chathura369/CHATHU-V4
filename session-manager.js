...
async function startSocket(id, entry) {
    try {
        ...
        if (entry.pairMode && entry.phoneNumber && !state.creds.registered) {
            const cleaned = entry.phoneNumber.replace(/[^0-9]/g, '');
            // Baileys needs a brief moment before requestPairingCode can be called
            setTimeout(async () => {
                try {
                    if (registry.get(id)?.status === 'Connected') return;
                    const code = await sock.requestPairingCode(cleaned);
                    const sessionEntry = registry.get(id);
                    if (sessionEntry) {
                        sessionEntry.pairCode = code;
                        sessionEntry.pairCodeExpiresAt = Date.now() + 60000; // 60s expiry
                        emit('session:paircode', { id, code, expiresAt: sessionEntry.pairCodeExpiresAt });
                        emit('session:update', { id, pairCode: code, pairCodeExpiresAt: sessionEntry.pairCodeExpiresAt, status: sessionEntry.status });
                        logger(`[Session ${id}] Pair code auto-requested for ${cleaned}: ${code}`);
                    }
                } catch (e) {
