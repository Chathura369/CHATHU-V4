...
function getCommonArgs() {
  ...
  if (process.execPath) {
    if (fs.existsSync(process.execPath)) {
      ...
      logger(`[yt-dlp] Node.js runtime candidate (process.execPath): ${process.execPath}`);
    } else {
      logger(`[yt-dlp] process.execPath does not exist on disk, skipping: ${process.execPath}`);
    }
  }
  try {
    ...
    if (found && fs.existsSync(found)) {
      ...
      logger(`[yt-dlp] Node.js runtime candidate (which node): ${found}`);
    } else if (found) {
      logger(`[yt-dlp] 'which node' returned a path that does not exist on disk, skipping: ${found}`);
    }
  } catch {}

  const nodePath = candidates.find(Boolean);
  if (nodePath) {
    _commonArgs = ["--js-runtimes", `node:${nodePath}`];
    logger(`[yt-dlp] Using Node JS runtime at ${nodePath} for YouTube extraction`);
  } else {
    _commonArgs = [];
    logger(`[yt-dlp] No valid Node.js runtime path found — omitting --js-runtimes, yt-dlp will use its own detection`);
  }
