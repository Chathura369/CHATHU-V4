...
const PROCESSED_MSG_LIMIT = 2000;
...
function loadCommands() {
  ...
  for (const file of files) {
    try {
      ...
      for (const cmd of cmds) {
        ...
        const commandName = String(cmd.name).trim().toLowerCase();
        if (!commandName) continue;

        if (registered.has(commandName)) {
          duplicateLogs.push(`[Handler] Duplicate command "${commandName}" in ${file}; already registered by ${registered.get(commandName)}.`);
          continue;
        }

        commands.set(commandName, cmd);
        registered.set(commandName, file);
        commandCount += 1;

        for (const rawAlias of cmd.aliases || []) {
          const alias = String(rawAlias || "").trim().toLowerCase();
          if (!alias || alias === commandName) continue;
          if (registered.has(alias)) {
            duplicateLogs.push(`[Handler] Duplicate alias "${alias}" for ${commandName} in ${file}; already registered by ${registered.get(alias)}.`);
            continue;
          }
          commands.set(alias, cmd);
          registered.set(alias, file);
          aliasCount += 1;
        }
      }
    } catch (err) {
  ...
  duplicateLogs.slice(0, 25).forEach((line) => logger(line));
  if (duplicateLogs.length > 25) {
    logger(`[Handler] ${duplicateLogs.length - 25} additional duplicate command/alias warnings suppressed.`);
  }
  logger(`[Handler] Successfully initialized ${commandCount} commands and ${aliasCount} aliases.`);
}
...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
    ...
    if (context.botEnabled === false) {
      const prefixForOwner = context.prefix || getPrefix();
      const maybeOwnerCommand = cmdText.startsWith(prefixForOwner);
      const ownerCandidate = msg.key.fromMe || isOwner(sender, ownerRefs);
      if (!maybeOwnerCommand || !ownerCandidate) return false;
    }
    ...
    if (!global.processedMsgQueue) global.processedMsgQueue = [];
    const msgId = `${context.sessionId || "__main__"}:${from}:${msg.key.id}`;
    if (global.processedMsgIds.has(msgId)) return false;
    global.processedMsgIds.add(msgId);
    global.processedMsgQueue.push(msgId);
    while (global.processedMsgQueue.length > PROCESSED_MSG_LIMIT) {
      global.processedMsgIds.delete(global.processedMsgQueue.shift());
    }
    ...
    if (!cmdText.startsWith(prefix)) {
      ...
      if (isAiAuto || isStdAuto) {
        ...
        if (aiCmd && typeof aiCmd.generateAIResponse === "function") {
          if (aiReplyLocks.has(from)) return false;
          const lock = (async () => {
            try {
              const now = Date.now();
              ...
              aiReplyLocks.delete(from);
            }
          })();
          aiReplyLocks.set(from, lock);
        }
      }
      return false;
    }
    ...
    if (cmd.ownerOnly && !isOwnerUser && !msg.key.fromMe) {
      await msgMgr.sendTemp(sock, from, "🔒 Owner only.", 4000);
      return true;
    }

    if (cmd.groupOnly && !isGroup) {
      await msgMgr.sendTemp(sock, from, "⚠️ This command only works in groups.", 4000);
      return true;
    }

    if (cmd.pmOnly && isGroup) {
      await msgMgr.sendTemp(sock, from, "⚠️ This command only works in private chat.", 4000);
      return true;
    }

    if (cmd.premiumOnly && !isOwnerUser && db.getUser(sender)?.premium !== true) {
      await msgMgr.sendTemp(sock, from, "💎 Premium users only.", 4000);
      return true;
    }
    ...
    const cooldownMs = Math.max(0, Number(cmd.cooldown || 0)) * 1000;
    if (cooldownMs > 0 && !isOwnerUser) {
      const cooldownKey = `${context.sessionId || "__main__"}:${from}:${sender}:${cmd.name}`;
      const lastRun = commandCooldowns.get(cooldownKey) || 0;
      const remaining = cooldownMs - (Date.now() - lastRun);
      if (remaining > 0) {
        await msgMgr.sendTemp(sock, from, `⏳ Wait ${Math.ceil(remaining / 1000)}s before using *${cmd.name}* again.`, 3000);
        return true;
      }
