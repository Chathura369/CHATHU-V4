...
function privacyWarning(key, enabled) {
    if (!enabled || !['antiDelete', 'antiViewOnce'].includes(key)) return '';
    return '\n\n⚠️ Privacy warning: this stores private media/messages for admin review.';
}
...
module.exports = [
    ...
    },
    {
        ...
        execute: async (sock, msg, from, args, cmdName, context) => {
            ...
            if (!service || !envVar) {
                return await msgMgr.send(sock, from, {
                    text: "⚠️ Usage: set API keys from your hosting dashboard/environment variables only.\nSupported: GEMINI_API_KEY, OPENROUTER_API_KEY, GROQ_API_KEY"
                }, { quoted: msg });
            }
            ...
            await msgMgr.send(sock, from, {
                text: `🔐 *${envVar}* must be updated in your host environment variables or dashboard secrets, not through WhatsApp chat.\nRestart the bot after changing the environment.`
            });
        }
    },
    {
        ...
        execute: async (sock, msg, from, num, quotedId, context) => {
            ...
            if (setting.type === 'action') {
                const envName = setting.service === 'gemini'
                    ? 'GEMINI_API_KEY'
                    : setting.service === 'openrouter'
                        ? 'OPENROUTER_API_KEY'
                        : 'GROQ_API_KEY';
                await msgMgr.send(sock, from, { text: `🔐 Update *${envName}* from your hosting dashboard/environment variables, then restart the bot. API keys are not accepted in WhatsApp chat.` }, { quoted: msg });
                return true;
            }
            ...
            if (setting.type === 'bool') {
                ...
                if (sent?.key?.id) {
                    settingsCache.set(sent.key.id, {
                        sender,
                        setting,
                        type: 'selection',
                        settings: cache.settings,
                        prefix: cache.prefix,
                        sessionId: cache.sessionId || context.sessionId || '__main__'
                    });
                    setTimeout(() => settingsCache.delete(sent.key.id), 300000);
                }
                return true;
            ...
            if (success) {
                await msgMgr.send(sock, from, { text: `✨ *${setting.label}* updated to: *${String(newVal).toUpperCase()}*${privacyWarning(setting.key, newVal)}` }, { quoted: msg });
                return true;
            }
            return false;
        }
    ...
    },
    {
        ...
        execute: async (sock, msg, from, args, cmdName, context) => {
            ...
            const arg = (args[0] || '').trim().toLowerCase();
            const sessionId = context.sessionId || '__main__';
            const current = getSessionSettings(sessionId).workMode || 'public';
            ...
            await updateSessionSetting(sessionId, 'workMode', arg);
            await sendReact(sock, from, msg, "✅");
