    ...
    try {
      ...
      if (cmdName === "imdb") {
        if (!config.OMDB_API_KEY) {
          return msgMgr.sendTemp(sock, from, "⚠️ OMDB_API_KEY is not configured.", 5000);
        }
        const { data } = await axios.get(`http://www.omdbapi.com/?t=${encodeURIComponent(q)}&apikey=${encodeURIComponent(config.OMDB_API_KEY)}`);
      ...
      if (cmdName === "weather") {
        if (!config.OPENWEATHER_API_KEY) {
          return msgMgr.sendTemp(sock, from, "⚠️ OPENWEATHER_API_KEY is not configured.", 5000);
        }
        const { data } = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(q)}&units=metric&appid=${encodeURIComponent(config.OPENWEATHER_API_KEY)}`);
      ...
      if (cmdName === "news") {
        if (!config.NEWS_API_KEY) {
          return msgMgr.sendTemp(sock, from, "⚠️ NEWS_API_KEY is not configured.", 5000);
        }
        const { data } = await axios.get(`https://newsapi.org/v2/everything?q=${encodeURIComponent(q)}&pageSize=5&apiKey=${encodeURIComponent(config.NEWS_API_KEY)}`);
        ...
        reply += "\n";
        reply += themeMgr.format("section", { title: `ʀᴇsුʟᴛs ғᴏʀ "${q}"` }, tCtx);
