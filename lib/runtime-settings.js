...
function getAutoRead() {
  return readSetting("autoRead", config.AUTO_READ) === true;
}
...
function getNsfwEnabled() {
  return readSetting("nsfwEnabled", config.NSFW_ENABLED) === true;
}
...
function getAutoViewStatus() {
  return readSetting("auto_view_status", false) === true;
}
